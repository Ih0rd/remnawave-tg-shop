...
def _hwid_limit_disabled(max_devices_value: Optional[object]) -> bool:
    if max_devices_value is None:
        return True
    try:
        return int(max_devices_value) <= 0
...
async def my_subscription_command_handler(
    ...
    expired_upgrades_count = await squad_upgrade_service.sync_expired_upgrades(session, event.from_user.id)
        ...
        if settings.squad_upgrade_offer and has_active_squad_upgrade is False:
            prepend_rows.append([
                InlineKeyboardButton(
                    ...
                    callback_data="main_action:buy_squad_upgrade",
                )
            ])
...
@router.callback_query(F.data == "main_action:buy_squad_upgrade")
async def buy_squad_upgrade_menu(
    callback: types.CallbackQuery,
    settings: Settings,
    i18n_data: dict,
    session: AsyncSession,
    ...
    for months in (1, 3, 6, 12):
        ...
        if rub_price is None and stars_price is None:
            continue
        rows.append([InlineKeyboardButton(
            text=get_text("squad_upgrade_period_button", months=months, rub_price=rub_price or "-", stars_price=stars_price or "-"),
...
@router.callback_query(F.data.startswith("squad_upgrade_period:"))
async def squad_upgrade_payment_methods_menu(
    callback: types.CallbackQuery,
    settings: Settings,
    i18n_data: dict,
):
    ...
    if enable:
        ...
        if not has_saved_card:
            try:
                ...
                pass
            try:
                await my_subscription_command_handler(callback, i18n_data, settings, panel_service, subscription_service, session, bot, device_package_service, squad_upgrade_service)
            except Exception:
                pass
            return
...
async def connect_command_handler(
    ...
    await my_subscription_command_handler(message, i18n_data, settings, panel_service, subscription_service, session, bot, device_package_service, squad_upgrade_service)
