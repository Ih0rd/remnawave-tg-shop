...
def get_promo_benefit_text(promo: PromoCode, i18n: JsonI18n, current_lang: str) -> str:
    ...
    if (promo.promo_type or "bonus_days") == "squad_upgrade":
        upgrade_days = promo.upgrade_days or 0
        if upgrade_days > 0:
            return _(
                "admin_promo_card_upgrade",
                default="🚀 Апгрейд подписки: <b>{upgrade_days} дн.</b>",
                upgrade_days=upgrade_days,
            )
        return _(
            "admin_promo_card_upgrade_legacy",
            default="🚀 Апгрейд подписки: <b>1 месяц (legacy)</b>",
        )
