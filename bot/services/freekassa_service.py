    ...
    async def webhook_route(self, request: web.Request) -> web.Response:
        ...
        async with self.async_session_factory() as session:
            ...
            try:
                ...
                if (payment.provider or "").endswith("-addon") and self.device_package_service:
                    ...
                    activation = {"end_date": await self.device_package_service.activate_paid_package(
                        session, payment.user_id, payment.payment_id, package_key=package_key, months=months
                    )}
                    referral_bonus = None
                    ...
                    activation = {"end_date": await self.squad_upgrade_service.activate_paid_upgrade(
                        session, payment.user_id, payment.payment_id, duration_days=months * 30
                    )}
                    referral_bonus = None
                else:
                    activation = await self.subscription_service.activate_subscription(
                        session, payment.user_id, months, float(payment.amount), payment.payment_id, provider="freekassa"
                    )
                    referral_bonus = await self.referral_service.apply_referral_bonuses_for_payment(
                        session, payment.user_id, months, current_payment_db_id=payment.payment_id, skip_if_active_before_payment=False
                    )
            ...
            if (payment.provider or "").endswith("-addon"):
                text = _("extra_devices_purchase_success", end_date=end_date_str)
            elif (payment.provider or "").endswith("-upgrade"):
                text = _("squad_upgrade_purchase_success", end_date=end_date_str)
            elif applied_days:
