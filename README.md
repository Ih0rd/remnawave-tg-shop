...
| `BANNERS_DIR` | (Опционально) Путь до директории баннеров меню внутри контейнера. По умолчанию `/app/banners` (в контейнере). | `/app/banners` |
...
### 🖼️ Баннеры пользовательских меню

Бот поддерживает **опциональные** баннеры (форматы: `jpg`, `jpeg`, `png`, `gif`, `webp`) для пользовательских экранов.

Имена файлов (без поддиректорий) должны соответствовать ключам меню:
- `main_menu.*` — главное меню
- `subscription_options.*` — страница покупки основной подписки
- `my_subscription.*` — страница «Моя подписка»
- `referral.*` — реферальная программа
- `promo_code.*` — ввод промокода
- `my_devices.*` — список устройств
- `buy_squad_upgrade.*` — покупка LTE апгрейда
- `buy_extra_devices.*` — покупка пакетов устройств

Пример для docker compose (чтобы хранить баннеры рядом с проектом):
```yaml
services:
  remnawave-tg-shop:
    volumes:
